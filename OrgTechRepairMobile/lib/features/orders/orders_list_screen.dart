              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final number = o['orderNumber']?.toString() ?? '';
                      final clientName = o['clientName']?.toString() ?? '';
                      final status = o['status']?.toString() ?? '';
                      return Card(
                        margin: const EdgeInsets.only(bottom: 8),
                        child: ListTile(
                          title: Text(number),
                          subtitle: Text('$clientName · $status'),
                          onTap: () => Navigator.pushNamed(context, '/orders/view', arguments: id),
                          trailing: canEdit ? IconButton(
                            icon: const Icon(Icons.edit),
                            onPressed: () => Navigator.pushNamed(context, '/orders/edit', arguments: id),
                          ) : null,
                        ),
                      );
                    },
      ...
      actions: [
        if (canEdit)
          IconButton(
            ...
            onPressed: () => Navigator.pushNamed(context, '/orders/create').then((_) => _load()),
          ),
      ],
    );
  }
